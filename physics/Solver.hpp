...
class Solver {
                ...
                if (body_list[i]->shape == ShapeType::CIRCLE) {
                    ...
                    if (body_list[j]->shape == ShapeType::CIRCLE) {
                        ...
                        if (Collisions::intersectCircles(obj1->position,
                                                         ...
                                                         obj2->radius,
                                                         manifold)) {
                            float obj1_ratio = obj2->is_static ? 1.f : 1.f / (1.f + obj1->mass() / obj2->mass());
                            float obj2_ratio = obj1->is_static ? 1.f : 1.f / (1.f + obj2->mass() / obj1->mass());

                            obj1->move(-manifold.normal * obj1_ratio * manifold.depth);
                            obj2->move(manifold.normal * obj2_ratio * manifold.depth);

                            Collisions::resolveCollision(*obj1, *obj2, manifold.normal, manifold.depth);
                        }
                    }
                    ...
                    if (body_list[j]->shape == ShapeType::POLYGON) {
                        ...
                        if (Collisions::intersectCircleAndPolygon(obj2->position,
                            ...
                            Collisions::resolveCollision(*obj2, *obj1, manifold.normal, manifold.depth);
                        }
                    }
                }
            }
 ...
 public:
    ...
    void setSubSteps(uint32_t sub_steps) {
