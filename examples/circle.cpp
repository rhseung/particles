...
//                 for (auto &object : solver.getBodyList()) {
//                     auto &circle = dynamic_cast<CircleBody &>(*object);
//                     if (std::abs(circle.position() - mouse) < 100)
//                         circle.setPosition(1000.0f * Math::normalize(circle.position() - mouse));
//                 }
//             }
//         }
//
//         if (solver.getBodyList() < max_objects_count && clock.getElapsedTime().asSeconds() >= object_spawn_delay) {
//             clock.restart();
//             Material materials[] = {
...
//                                                            materials[solver.getBodyCount() % 13], getRainbow(t)));
