...
int main() {
    ...
    Solver solver{{0.0f, 1500.0f}, 4, frame_rate};
    ...
    // TODO: 길이와 값을 전부 현실에 동기화시키기 e.g. 중력은 9.81로.
    ...
    evm.addMousePressedCallback(sf::Mouse::Right, [&](sf::Event event) {
    ...
    while (window.isOpen()) {
        for (auto &obj : solver.getBodyList()) {
            if ((obj->position.x < 0 || obj->position.x > window_size.x) && (obj->position.y < 0 || obj->position.y > window_size.y)) {
